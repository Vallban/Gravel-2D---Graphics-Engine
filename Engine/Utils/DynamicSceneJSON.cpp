...
void DynamicSceneJSON::crearEscenaEnJSON(const std::string& rutaArchivo) {
    std::ofstream archivo(rutaArchivo);
    archivo << escena.dump(4);
    archivo.close();
}

void DynamicSceneJSON::anyadirImagenEnJSON(const std::string& id, const std::string& ruta, int x, int y){
